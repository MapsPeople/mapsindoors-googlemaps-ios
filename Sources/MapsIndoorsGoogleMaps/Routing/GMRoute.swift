...
@_spi(Testable) public struct GoogleRoute: Codable {
    ...
    var asMPRoute: MPRoute {
        ...
        var aggregatedDuration = 0.0
        
        for googleLeg in (self.legs ?? []) {
            
            let mpLeg = MPRouteLegInternal()
            
            aggregatedDistance += (googleLeg.distance?.value ?? 0.0)
            aggregatedDuration += (googleLeg.duration?.value ?? 0.0)
        ...
        mpRoute.duration = aggregatedDuration as NSNumber
        mpRoute.distance = aggregatedDistance as NSNumber
